...
參考 `commandments.md` 了解統一的戒律體系：
- **天條**（Universal）：8 條通用戒律，所有道兵共守，含 2 條不赦之條
- **職律**（Role-specific）：依法號而定的額外戒律
- **壇律**（Ceremony-specific）：法師為特定科儀定義的臨時戒律
- **豁免機制**：法師可豁免特定戒律（不赦之條除外）

### 加持符系統

參考 `enhancements.md` 了解六道加持符：
- **自省訣**：完成後自動驗證功德
- **通報訣**：多步驟進度回報
- **靜心訣**：關鍵決策前先推演再行動
- **結界符**：封印路徑、法器、網路存取範圍
...
參考 `merit-record.md` 了解審計與評分體系：
- **召喚記錄**：每次敕令道兵的完整記錄
- **功過考核**：五維度評分（天命達成/法力效率/品質/戒律/渡劫）
- **科儀總結**：大醮後的群將功過一覽
- **月報統計**：長期趨勢追蹤

### 演法壇（Dry Run）

參考 `simulator.md` 了解如何在正式開壇前預演：
- 用 mock 資料走完整科儀
- 驗證壇位銜接與法物格式
- 測量耗時與法力消耗
- 支援局部演法（只測特定壇位銜接）
...
參考 `inheritance.md` 了解符籙繼承機制：
- **母符**：定義專案級共用設定（壇場、戒律、門風、結界）
- **子符**：只寫差異部分，其餘繼承母符
- 安全只增不減——子符可追加戒律，不可移除
- 最多三層傳承（祖符→母符→子符）
